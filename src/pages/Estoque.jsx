...
const ST = { ok:{ bg:'var(--verde-bg)', cor:'var(--verde)', ic:'✅ OK' }, baixo:{ bg:'var(--laran-bg)', cor:'var(--laranja)', ic:'⚠️ Baixo' }, critico:{ bg:'var(--verm-bg)', cor:'var(--vermelho)', ic:'🔴 Crítico' } }
...
export default function Estoque() {
  ...
  function abrirEditar(item) {
    setForm({ produto:item.produto, categoria:item.categoria||'', qtd_atual:item.qtd_atual, qtd_minima:item.qtd_minima, unidade:item.unidade||'un', observacao:item.observacao||'' })
  ...
  async function salvar() {
    ...
    if (editando) {
      ...
      if (!error) {
        await supabase.from('movimentacoes_estoque').insert({ estoque_id:editando, tipo:'edicao', quantidade:0, motivo:`Edição por ${nomeUser}`, usuario_nome:nomeUser, usuario_id:user?.id })
      }
    } else {
      const res = await supabase.from('estoque').insert(payload).select('id').single()
      error = res.error
      if (!error) {
        await supabase.from('movimentacoes_estoque').insert({ estoque_id:res.data.id, tipo:'cadastro', quantidade:parseFloat(form.qtd_atual)||0, motivo:`Cadastrado por ${nomeUser}`, usuario_nome:nomeUser, usuario_id:user?.id })
      }
    }
  ...
  async function remover(id, produto) {
    ...
    await supabase.from('movimentacoes_estoque').insert({ estoque_id:id, tipo:'exclusao', quantidade:0, motivo:`Excluído por ${nomeUser}`, usuario_nome:nomeUser, usuario_id:user?.id })
  ...
  return (
    <div className="animate-in">
      <div className="page-header">
        <div><h1 className="page-title">📦 Controle de Estoque</h1><p className="page-subtitle">Produtos, movimentações e auditoria completa</p></div>
        <div className="page-actions">
          <button className="btn btn-ghost btn-sm" onClick={() => gerarRelatorioEstoquePDF(lista)}>📄 PDF</button>
      ...
      {(criticos > 0 || baixos > 0) && (
        <div className="alert alert-red" style={{ marginBottom:18 }}>
          ...
          <div>{criticos > 0 && <><strong>{criticos} item(s) crítico(s)</strong> — estoque zerado. </>}{baixos > 0 && <><strong>{baixos} item(s) baixo(s)</strong> — abaixo do mínimo.</>}</div>
        </div>
      )}
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:18 }}>
            {[{ic:'📦',v:lista.length,l:'Total',c:'var(--azul)'},{ic:'⚠️',v:baixos,l:'Baixo',c:'var(--laranja)'},{ic:'🔴',v:criticos,l:'Crítico',c:'var(--vermelho)'}].map(s => (
              <div key={s.l} style={{ background:'white', borderRadius:12, padding:'14px 16px', border:'1px solid var(--borda)', boxShadow:'var(--shadow)', display:'flex', alignItems:'center', gap:12 }}>
                ...
                <div><div style={{ fontFamily:'var(--font-display)', fontSize:24, fontWeight:700, color:s.c, lineHeight:1 }}>{s.v}</div><div style={{ fontSize:10.5, color:'var(--cinza)', fontWeight:700, textTransform:'uppercase', letterSpacing:.4, marginTop:2 }}>{s.l}</div></div>
              </div>
            ))}
          </div>
