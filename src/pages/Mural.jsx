...
export default function Mural() {
  ...
  const [form, setForm] = useState({ titulo: '', conteudo: '', tipo: 'aviso', fixado: false })
  ...
  async function salvar() {
    ...
    if (error) toast('Erro ao publicar: ' + error.message, 'error')
    else {
      toast('Aviso publicado no mural!')
      setModalOpen(false)
      setForm({ titulo: '', conteudo: '', tipo: 'aviso', fixado: false })
      loadAvisos()
    }
    setSaving(false)
  }
  ...
  async function remover(id) {
    ...
    await supabase.from('mural').update({ ativo: false }).eq('id', id)
    ...
    <div className="animate-in">
      <div className="page-header">
        ...
        <div className="page-actions">
          ...
          {isCoord && (
            <button className="btn btn-gold" onClick={() => setModalOpen(true)}>+ Novo Aviso</button>
          )}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          {isCoord && <button className="btn btn-gold" onClick={() => setModalOpen(true)}>Publicar primeiro aviso</button>}
        </div>
      ) : (
              ...
              <div className="mural-meta">
                ...
                {isCoord && (
                  <div style={{ display: 'flex', gap: '6px', marginLeft: 'auto' }}>
                    ...
                    <button className="btn btn-sm btn-danger" onClick={() => remover(a.id)} style={{ padding: '4px 8px', fontSize: '13px' }}>🗑</button>
                  </div>
                )}
              </div>
            </div>
